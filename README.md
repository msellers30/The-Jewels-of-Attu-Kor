...
The TRS-80 disks were propriatary AFAIK and we had no way of saving the game code, but thankfully Shannon had the foresight of printing it out. He kept the printout in his bauble storage case for many years until one day he gave it to me and I scanned it and used the OCR from the scanner to turn it back into the original BASIC program. This scanner output is the [original game](docs/original-game.txt) file. I converted it to C# (back when .NET Framework 3.5 was a thing) and recently updated it to .NET 10 Core.
